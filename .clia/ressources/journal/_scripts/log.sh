#!/usr/bin/env bash
# Description: La ressource journal — ce qu'elle porte.
# Périmètre: dépôt
# Signature: log ls
#
# La commande de la ressource « journal ».
#
...
#
#   le nom du fichier est le nom de la commande — log.sh donne
#   « clia log », et le noyau l'emporte en cas d'homonymie ;
#
...
RESSOURCE_REL='_ressources/journal'
...
manuel() {
  cat <<'FIN' | _clia_man clia-log 1 "Manuel de l'utilisateur clia"
NOM
clia-log - la ressource journal

SYNOPSIS
clia log ls

DESCRIPTION
Journal est une ressource informationnelle de ce dépôt. Elle dit
...
FICHIERS
_ressources/journal/journal.yaml
       La définition de la ressource.

_ressources/journal/primitives/
       Ce à partir de quoi ses livrables sont produits.

_ressources/journal/_scripts/log.sh
       Ce fichier.
...
lister() {
  ...
  if (( nb == 0 )); then
    _clia_msg "journal ne porte aucune primitive"
...
case "$VERBE" in
  '')
    _clia_msg "clia log attend un verbe"
    _clia_detail "l'usage : clia log --help"
    exit 2 ;;
