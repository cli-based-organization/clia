...
type: analyse
version: 0.1.0
title: "Usage de l'IA dans le projet au regard du principe de nécessité"
date: 2026-07-18
---

# ANL-012 - Usage de l'IA dans le projet au regard du principe de nécessité
...
- **Référence** : `FND-010-capacites-modeles-ia-usage` (critère IA vs automatisation) ; principe de session « n'utiliser l'IA que lorsque c'est absolument nécessaire ; le reste => automatiser ; permettre à l'humain d'exécuter à la demande ou d'inspecter n'importe quelle étape ».
...
**C3. Des mécaniques déterministes sont encore portées par l'agent, en tension avec le principe.** Plusieurs opérations que l'agent réalise « à la main » sont entièrement spécifiables et vérifiables, donc candidates à l'automatisation par `clia` :
   ...
   - **la cohérence de format** des ressources vivantes (lignes « Statut : » / « Version : » lues par `clia`, voir `ANL-005-besoin-req-spec-ressources-livrables`) : vérifiable par machine.
...
Face à la `FND-010-capacites-modeles-ia-usage` et au principe de session :
