...
end-at: 2026-07-31T15:46:07-04:00
---
...
## 40. [traitement des objections]

### objection 1

Implémenter un test et testé. Ce qui a été demander était seulement de ne pas formaliser prématurément le processus de tests

### objection 2

La réponse est les 2 à la fois !

en mode dev, le cli clia pointe vers le code source. Il faut maintenir cela en mode --dev

Cependant, la commande `clia setup ...` ne devrait pas être réimplémentée. Elle devrait faire appel au script setup.sh considéré comme une extention/plugin de clia

### objection 3

Non. la connaissance de la version est essentielle. Proposer, dans ce plan, d'implémenter un mécanisme de découverte des versions disponibles.

Attention avec la notion de version. Dans le repo de clia, la version "métier" EST la version installée. Ajouter une marque dans le repo utilisant clia afin d'identifier la version installée.

ou bien, déduire la version installée à partir de la version de chacun des harnais

## 41. [objection de l'humain]

dans le PLN-019, il est dit que l'outils (le cli clia) est installé. CE N'EST PAS CE QU'IL FAUT FAIRE.

Le cli clia ne fait pas partie des harnais et assets à installé avec la commande `clia setup init ...`. Il ne faut pas l'installé. Le script clia est installé "globalement" et peut être utilisé de n'importe où par l'utilisateur. 

Ce cli repère si le répertoire courrant est un repo compatible avec clia. EXACTEMENT comme ce qui est fait dans tda.

TODO: réévaluer le plan PLN-019 au regard de l'objection soulevée


## 42. [implémentation]

Implémenter le plan PLN-018, puis PLN-019


## 43. [feedback humain]

Utiliser `clia setup CMD` et non pas des commandes de premier niveau `clia CMD`.

Accepter les propositions par défaut. Implémenter PLN-018 et PLN-019


## 44. [documentation] Écrire un readme incluant comment installer clia et comment initialiser un nouveau repo

## 45. [documentation] pourquoi `clia setup CMD` n'est pas documenté? Corriger ce bogue
