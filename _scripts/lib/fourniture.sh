...
# ---------------
#
#   _ressources/<nom>/features/<f>.md        frontmatter, puis le corps
#   _ressources/<nom>/skills/<s>/SKILL.md    ou _ressources/<nom>/skills/<s>.md
#   _ressources/<nom>/_scripts/<cmd>.sh      ses « # Signature: » font les verbes
#
...
_clia_f_ressources() {
  ...
  for racine in "${CLIA_SOURCE_DIR:-}" "$depot"; do
    [[ -n "$racine" && -d "$racine/_ressources" ]] || continue
...
_clia_f_features() {
  ...
  while IFS="$_CLIA_SEP" read -r prefixe nom racine; do
    [[ -n "$nom" ]] || continue
    for f in "$racine/_ressources/$nom/features"/*.md; do
      [[ -f "$f" ]] || continue
      n=$(basename "$f" .md)
...
_clia_f_skills() {
  ...
  while IFS="$_CLIA_SEP" read -r prefixe nom racine; do
    [[ -n "$nom" ]] || continue
    for s in "$racine/_ressources/$nom/skills"/*; do
      [[ -e "$s" ]] || continue
      if [[ -d "$s" ]]; then
        n=$(basename "$s")
        fichier="$s/SKILL.md"
...
_clia_f_scripts() {
  ...
  while IFS="$_CLIA_SEP" read -r prefixe nom racine; do
    ...
    fichier="$racine/_ressources/$nom/_scripts/$commande.sh"
    [[ -f "$fichier" ]] || continue
    vus=''
