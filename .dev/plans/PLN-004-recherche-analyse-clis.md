...
type: plan
version: 0.1.0
title: "Recherche, analyse et outillage méthodologique des CLI"
status: exécuté
---
...
3. **FND-007-conventions-cli** - `.dev/fondations/FND-007-conventions-cli.md` : recherche de fondation théorique et sourcée sur les conventions de CLI (arguments, options/flags, sous-commandes, codes de sortie, stdout/stderr, aide, POSIX/GNU, guides de référence reconnus). Produit par `skl-002`.
4. **ANL-001-etat-clis-existants** - `.dev/analyses/ANL-001-etat-clis-existants.md` : inventaire et analyse des scripts bash / CLI trouvés récursivement sous `/home/jvtrudel/git/`, confrontés à FND-007-conventions-cli (écarts, bonnes pratiques observées, anti-patterns). Produit par `skl-012`.
...
### 3. FND-007-conventions-cli - fondation théorique sur les conventions de CLI
...
### 5. ANL-001-etat-clis-existants - analyse de l'existant au regard de FND-007-conventions-cli
Via `skl-012` : inventaire structuré (dépôt, chemin, rôle), puis confrontation à FND-007-conventions-cli (conformité, écarts, bonnes pratiques, anti-patterns). Base empirique de la convention (étape 7).
...
À partir de FND-007-conventions-cli, ANL-001-etat-clis-existants et des nouveaux skills : ADR-002 (décision de convention et alternatives écartées), SPEC-001 (structure concrète d'un CLI bash conforme : flags standards, gestion des erreurs, aide, sortie), REQ-001 (exigences que tout CLI bash du dépôt doit respecter).
...
- **Ordre d'exécution** : définir le type analyse et son skill (étapes 1 à 2) avant de produire ANL-001-etat-clis-existants (étape 5) ; produire FND-007-conventions-cli avant ANL-001-etat-clis-existants puisque l'analyse s'y confronte.
...
- `FND-007-conventions-cli.md` : fondation théorique sourcée sur les conventions de CLI.
- `ANL-001-etat-clis-existants.md` : analyse du corpus de CLI locaux, confrontée à FND-007-conventions-cli.
