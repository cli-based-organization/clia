# ANL-2026-07-18 - Intérêt du Model Context Protocol (MCP) pour clia
...
- **Référence** : `FND-2026-07-18-mcp-model-context-protocol` ; `FND-2026-07-18-capacites-modeles-ia-usage` (critère « IA/complexité seulement si nécessaire ») ; `ANL-2026-07-18-usage-ia-projet`.
...
**C6. Confrontation au principe « IA/complexité seulement si nécessaire ».** `FND-2026-07-18-capacites-modeles-ia-usage` et `ANL-2026-07-18-usage-ia-projet` posent : n'introduire une couche IA/complexe que si le besoin l'exige, automatiser/simplifier le reste. MCP est précisément une couche d'intégration IA. L'ajouter **sans besoin actuel** contredit ce principe et alourdirait un composant dont la valeur est la **simplicité déterministe**.
...
- `FND-2026-07-18-mcp` : la valeur de MCP est la **portabilité inter-clients** et l'**accès structuré**. Or `clia` sert aujourd'hui un seul client (l'agent Claude Code) via Bash, avec un accès déjà découvrable. Le bénéfice différenciant de MCP n'est donc pas activé.
...
- `FND-2026-07-18-capacites-modeles-ia-usage` : ajouter une couche IA non nécessaire contredit « IA/complexité seulement si nécessaire ».
